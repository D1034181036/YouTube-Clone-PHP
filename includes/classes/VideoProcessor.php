...
class VideoProcessor{
	...
	private $con;
	private $sizeLimit = 1024000000; //1024M Bytes
	...
	public function upload($videoUploadData){
		...
		//uniqid() = 不重複隨機碼, basename() = 取不帶路徑的文件名
		//upload/videos/5e43da92dd5e3dog_playing.flv
	...
	private function processData($videoData, $filePath){
		$videoType = pathInfo($filePath, PATHINFO_EXTENSION); //取得副檔名
	...
	public function generateThumbnails($filePath){
		...
		for($num = 1; $num <= $numThumbnails; $num++){
			...
			$interval = ($duration * 0.8) / $numThumbnails * $num; // 取影片0.8長度後split成三份
			$fullThumbnailPath = "$pathToThumbnail/$videoId-$imageName";

			$cmd = "$this->ffmpegPath -i $filePath -ss $interval -s $thumbnailSize -vframes 1 $fullThumbnailPath 2>&1"; //ss $interval秒截圖
