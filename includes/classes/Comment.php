	...
	class Comment{
		...
		public function like(){
            ...
            if($this->wasLikedBy() > 0){
                ...
                $query->bindParam(":commentId", $id);
                $query->execute();
                $count = $query->rowCount(); //如果有刪除 -> return 1

                $query = $this->con->prepare("INSERT INTO likes(username, commentId, videoId) VALUES (:username, :commentId, :videoId)");
                $query->bindParam(":username", $username);
                $query->bindParam(":commentId", $id);
        ...
        public function dislike(){
            ...
            if($this->wasDislikedBy() > 0){
                ...
                $query = $this->con->prepare("INSERT INTO dislikes(username, commentId, videoId) VALUES (:username, :commentId, :videoId)");
                $query->bindParam(":username", $username);
                $query->bindParam(":commentId", $id);
                $query->bindParam(":videoId", $videoId);
                $query->execute();
